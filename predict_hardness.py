...
file_path = "The calculation of features.xlsx"
...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        ...
        sheet.cell(row=row_num, column=18).value = enthalpy
   # Save the workbook
    wb.save(filename)
# Replace 'The calculation of features.xlsx' with the actual filename
calculate_feature('The calculation of features.xlsx')
...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):       
        ...
        sheet.cell(row=row_num, column=19).value = round(daierta, 4)
...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        ...
        # Write the result to the 'T' column of the current row
        sheet.cell(row=row_num, column=20).value =round(VEC, 4)
...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):       
        ...
        # Write the result to the 'T' column of the current row
        sheet.cell(row=row_num, column=21).value = round(number_of_itinerant_electrons, 4)
        sheet.cell(row=row_num, column=23).value = round(average_deviation_of_number_of_itinerant_electrons, 4)
...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):       
        ...
        # Write the result to the 'T' column of the current row
        sheet.cell(row=row_num, column=22).value = round(atom_radius, 4)
        sheet.cell(row=row_num, column=24).value = round(average_deviation_of_atom_radius, 4)
...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):       
        ...
        # Write the result to the 'T' column of the current row
        sheet.cell(row=row_num, column=25).value = round(average_deviation_of_Young_modulus, 4)
...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):       
        ...
        average_deviation_of_Shear_Modulus = (
            abs(r_values[0] - Shear_Modulus) * row[0] + abs(r_values[1] - Shear_Modulus) * row[1] +
            abs(r_values[2] - Shear_Modulus) * row[2] + abs(r_values[3] - Shear_Modulus) * row[3] +
            abs(r_values[4] - Shear_Modulus) * row[4] + abs(r_values[5] - Shear_Modulus) * row[5] +
            abs(r_values[6] - Shear_Modulus) * row[6] + abs(r_values[7] - Shear_Modulus) * row[7] +
            abs(r_values[8] - Shear_Modulus) * row[8] + abs(r_values[9] - Shear_Modulus) * row[9] +
            abs(r_values[10] - Shear_Modulus) * row[10] + abs(r_values[11] - Shear_Modulus) * row[11] +
            abs(r_values[12] - Shear_Modulus) * row[12] + abs(r_values[13] - Shear_Modulus) * row[13] +
            abs(r_values[14] - Shear_Modulus) * row[14]
        ) * 0.01
         
        # Write the result to the 'T' column of the current row
        sheet.cell(row=row_num, column=26).value = round(average_deviation_of_Shear_Modulus, 4) 
    # Save the workbook
    wb.save(filename)
...
test_data = pd.read_excel('The calculation of features.xlsx')    
...
test_data.to_excel('The calculation of features.xlsx', index=False)