...
def calculate_feature(filename):
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        ...
        Electronegativity_difference = ((
            (r_values[0]-Pauling_Electronegativity) ** 2 * row[0] + ((r_values[1]-Pauling_Electronegativity))**2 * row[1] +
            ((r_values[2]-Pauling_Electronegativity))**2 * row[2] + ((r_values[3]-Pauling_Electronegativity))**2 * row[3] +
            ((r_values[4]-Pauling_Electronegativity))**2 * row[4] + ((r_values[5]-Pauling_Electronegativity))**2 * row[5] +
            ((r_values[6]-Pauling_Electronegativity))**2 * row[6] + ((r_values[7]-Pauling_Electronegativity))**2 * row[7] +
            ((r_values[8]-Pauling_Electronegativity))**2 * row[8] + ((r_values[9]-Pauling_Electronegativity))**2 * row[9] +
            ((r_values[10]-Pauling_Electronegativity))**2 * row[10] + ((r_values[11]-Pauling_Electronegativity))**2 * row[11] +
            ((r_values[12]-Pauling_Electronegativity))**2 * row[12] + ((r_values[13]-Pauling_Electronegativity))**2 * row[13] +
            ((r_values[14]-Pauling_Electronegativity))**2 * row[14]
        ) * 0.01)**0.5
        sheet.cell(row=row_num, column=31).value = round(Electronegativity_difference, 4)
    # Save the workbook
    wb.save(filename)
